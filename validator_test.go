...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				v: struct {
					SliceString []string `validate:"max:3"`
				}{
					SliceString: []string{"aba", "ca", "ba"},
				},
			},
			wantErr: false,
		},
		{
			...
			args: args{
				v: struct {
					SliceString []string `validate:"min:2"`
				}{
					SliceString: []string{"a", "", "aba", "ca", "ba"},
				},
			},
			wantErr: true,
			checkErr: func(err error) bool {
		...
		},
		{
			...
			args: args{
				v: struct {
					SliceString []string `validate:"in:abacaba,cat"`
				}{
					SliceString: []string{"aba", "ca", "ba", "cat", "dog"},
				},
			},
			wantErr: true,
			checkErr: func(err error) bool {
		...
		},
		{
			...
			args: args{
				v: struct {
					SliceString []string `validate:"len:3"`
				}{
					SliceString: []string{"aba", "ca", "ba", "cat", "dog"},
				},
			},
		...
		{
			name: "wrong range (strings)",
			args: args{
				v: struct {
					SliceString []string `validate:"range:3,5"`
				}{
					SliceString: []string{"a", "bb", "ccc", "dddd", "eeeee"},
				},
			},
			wantErr: true,
			checkErr: func(err error) bool {
		...
		{
			name: "wrong range (integers)",
			args: args{
				v: struct {
					SliceInt []int `validate:"range:3,5"`
				}{
					SliceInt: []int{1, 2, 3, 4, 5},
				},
			},
